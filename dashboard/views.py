...
def notes(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            notes.save()
        messages.success(request,f"Notes Created from {request.user.username} Successfully!!")
    else:
        form = NotesForm()
...
def delete_homework(request,pk=None):
    homework = Homework.objects.get(id=pk)
    messages.success(request,f'{homework.title} homework deleted by {request.user.username}')
    homework.delete()
    return redirect('homework')
...
def delete_todo(request,pk=None):
    todo = Todo.objects.get(id=pk)
    messages.success(request,f'{todo.title} deleted by {request.user.username}')
    todo.delete()
    return redirect('todo')
